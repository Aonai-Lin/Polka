...
function generateAlternatePolka(frame:FrameNode, width: number, height: number, size: number, dotColor: RGB){
    // 设置间隔和起始位置
...
// 生成相同大小的圆点
function generateEvenPolka(frame:FrameNode, width: number, height: number, size: number, dotColor:RGB){
    // 设置初始位置和间隔
    const xSpacing = size * 2;
    const ySpacing = size * 2;
    let x = size / 2;
...
// 生成南瓜样式圆点
function generatePumpkinPolka(frame:FrameNode, width: number, height: number, size: number, dotColor:RGB){
    // 设置初始位置和间隔
...
function generateRowOfDots(frame:FrameNode, y: number, width: number, dotSize: number, dotColor: RGB){
    let x = dotSize * 0.2;
